...
function RouteComponent() {
  const task = useRouterState().location.state.task;
  if (!task) {
    return <div>Временный баг</div>;
  }
  return (
    <div className="p-4 ">
        ...
        <div className=" text-black flex items-start gap-4">
          <div>
            <h1 className="text-2xl font-bold mb-4  font-futura-heavy ">
              Миссия: {task.mission_title}
            </h1>
            <h2 className="text-xl font-bold mb-4">Цель: {task.goal_title}</h2>
            <h3 className="text-lg font-bold mb-4">
              Название задачи: {task.title}
            </h3>
            <p className="mb-2">Описание задачи: {task.description}</p>
            <p className="mb-2">Навык: {task.skill_title}</p>
            <p className="mb-2">Место: {task.online || "Нету"}</p>
            <p className="mb-2">Срок исполнения: {task.deadline || "Нету"}</p>
            <p className="mb-2">
